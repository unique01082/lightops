...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      style={{ background: checked ? 'var(--accent)' : 'var(--switch-background)' }}
    >
      <motion.div
...
export function AdvancedOptionsPanel({
  ...
  return (
    ...
    >
      <div className="px-4 py-3 border-b" style={{ borderColor: 'var(--glass-divider)' }}>
        <h2 className="text-white" style={{ fontFamily: 'var(--font-heading)' }}>{t('advanced.title')}</h2>
      </div>

      <div className="px-4 py-3 space-y-3">
        <div className="flex items-center justify-between">
          <label className="text-sm" style={{ color: 'var(--text-primary)' }}>{t('advanced.recursive')}</label>
          <Toggle checked={recursiveScan} onChange={onRecursiveScanChange} />
        </div>

        <div>
          <label className="block text-xs mb-2" style={{ color: 'var(--text-secondary)' }}>{t('advanced.fileOp')}</label>
        ...
        <div className="flex items-center justify-between">
          <label className="text-sm mr-4" style={{ color: 'var(--text-primary)' }}>{t('advanced.organizeByDate')}</label>
          <Toggle checked={organizeByDate} onChange={onOrganizeByDateChange} />
        </div>

        <div className="flex items-center justify-between">
          <label className="text-sm mr-4" style={{ color: 'var(--text-primary)' }}>{t('advanced.onlyPaired')}</label>
