  ...
  useEffect(() => {
    ...
    switch (format) {
      case 'YYYYMMDD_HHMMSS_NNNN':
        filename += '20240315_143022_' + num; break;
      case 'YYYY-MM-DD_HH-MM-SS_NNNN':
        filename += '2024-03-15_14-30-22_' + num; break;
      case 'YYYYMMDD_NNNN':
        filename += '20240315_' + num; break;
      case 'NNNN':
        filename += num; break;
    }
    filename += '.nef';
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2.5">
        <div>
          <label className="block text-xs mb-1" style={{ color: 'var(--text-secondary)' }}>Prefix</label>
          <input
            type="text"
            value={prefix}
            onChange={(e) => onPrefixChange(e.target.value)}
            placeholder="e.g. hanoi_"
        ...
        <div>
          <label className="block text-xs mb-1" style={{ color: 'var(--text-secondary)' }}>Filename Format</label>
        ...
        <div>
          ...
          <input
            type="number"
            value={startNumber}
            onChange={(e) => onStartNumberChange(Math.max(1, Math.min(99999, parseInt(e.target.value) || 1)))}
            min="1" max="99999"
            className="w-full px-3 py-2 rounded-lg border backdrop-blur-sm text-sm"
            style={{ background: 'var(--input-background)', borderColor: 'var(--glass-border)', color: 'var(--text-primary)' }}
          />
        ...
        <div>
          <label className="block text-xs mb-1" style={{ color: 'var(--text-secondary)' }}>Preview</label>
          <motion.div
            animate={{ backgroundColor: shouldHighlight ? 'rgba(139, 92, 246, 0.2)' : 'rgba(255, 255, 255, 0.05)' }}
            transition={{ duration: 0.3 }}
            className="px-3 py-2 rounded-full border text-sm"
            style={{ borderColor: 'var(--glass-border)', fontFamily: 'var(--font-mono)', color: 'var(--accent)' }}
          >
            {preview}
          </motion.div>
        </div>
